var widthMatrix = document.getElementById("Tsne").offsetWidth,
    heightMatrix = document.getElementById("Tsne").offsetHeight;
    ...
    d3.csv("data/box_calc_rank.csv").then((data) => {
        d3.csv("data/fpmtype4.csv").then((data2) => {
            d3.csv("data/fpm02.csv").then((data3) => {
                ...
                let sw = 70;
                svgMatrix.selectAll("#rc")
                    .attr('id', 'rc')
                    .data(rc)
                    .enter().append("rect")
                    .attr('x', d => {
                        return parseInt(d / 6) * sw + 180;
                    })
                    .attr('y', d => {
                        return (d % 6) * sw + 15;
                    })
                ...
                svgMatrix.selectAll("#cc")
                    .attr('id', 'cc')
                    .data(cd)
                    .enter().append("circle")
                    .attr('cx', (d, i) => {
                        return parseInt(i / 6) * sw + sw / 2 + 180;
                    })
                    .attr('cy', (d, i) => {

                        return (i % 6) * sw + sw / 2 + 15;
                    })
                    .attr('r', d => rScale(d))
                    .attr('fill', 'gray')
                    .attr('fill-opacity', 0.5);
                // console.log(typeCount)
                // svgMatrix.selectAll("#rl")
                //     .attr("id", 'rl')
                //     .data(typeC)
                //     .enter().append('rect')
                //     .attr('y', (d, i) => {
                ...
                svgMatrix.selectAll("#rl")
                    ...
                    .attr('x', (d, i) => {
                        // console.log(d)
                        return 180 - typeScale(d) - 0.5;
                    })
                    .attr('y', (d, i) => {
                        return parseInt(i) * sw + sw / 2 - 10 + 15;
                    })
                    .attr('height', 20)
                ...
                for (let i in typeSimilar) {
                    ...
                    let dd = {
                        source: {
                            y: typeSimilar[i].source * sw + sw / 2 + 10,
                            x: 600
                        },
                        target: {
                            y: typeSimilar[i].target * sw + sw / 2 + 10,
                            x: 600
                        }
                    };
                    var dx = dd.target.x - dd.source.x, //增量
                        dy = dd.target.y - dd.source.y,
                        dr = Math.sqrt(dx * dx + dy * dy) * 0.6;
                    var dp = "M" + dd.source.x + "," +
                        dd.source.y + "A" + dr + "," +
                        dr + " 0 0,1 " + dd.target.x + "," +
                        dd.target.y;
                    let kg = svgMatrix.append('g')
                ...
                for (let i in sim) {
                    ...
                    // break;
                    if (sim[i].source > sim[i].target) {
                        let temp = sim[i].source;
                        sim[i].source = sim[i].target;
                        sim[i].target = temp;
                    }
                    
                    let dd = {
                        target: {
                            y: sim[i].source * sw + sw / 2 + 10,
                            x: 100
                        },
                        source: {
                            y: sim[i].target * sw + sw / 2,
                            x: 100
                        }
                    };
                    var dx = dd.target.x - dd.source.x, //增量
                        dy = dd.target.y - dd.source.y,
                        dr = Math.sqrt(dx * dx + dy * dy) * 0.52;
                    var dp = "M" + dd.source.x + "," +
                        dd.source.y + "A" + dr + "," +
                        dr + " 0 0,1 " + dd.target.x + "," +
                        dd.target.y;
